...
first_death = min(which(data_Nouvellet$New_deaths > 0))
data_Nouvellet <- tail(data_Nouvellet, -first_death)

data.in <- list(N = nrow(data_Nouvellet),
                T = min(which(data_Nouvellet$date >= first_cutoff)),
...
```{r}
mcmc_trace(fit1, pars=c('beta1', 'beta2'))

R <- get_posterior_mean(fit1, pars='R')
RD <- get_posterior_mean(fit1, pars='RD')
plot(R[1:235,4], type='l', col='red')
lines(RD[1:235,4], col='blue')
